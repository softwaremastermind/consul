...
\*For `HTTPS` and `gRPC(s)` the ports specified in the table
are recommendations.
...
**gRPCs API**. Currently gRPC over TLS is
only used to expose the xDS API to Envoy proxies. It is [on by default](https://developer.hashicorp.com/consul/docs/agent/config/config-files#grpc_tls_port), and we recommend using 8503 for grpc_tls as your conventional gRPC port number, as it allows some tools to work automatically. 
