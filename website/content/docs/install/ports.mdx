...
**gRPCs API** . Currently gRPCs is
only used to expose the xDS API to Envoy proxies **over TLS**. It is on by default, and we recommend using 8503 for grpc_tls as your conventional gRPC port number, as it allows some tools to work automatically.
...
the Serf WANWe recommend using 8503 for grpc_tls as your conventional gRPC port number, as it allows some tools to work automatically.We recommend using 8503 for grpc_tls as your conventional gRPC port number, as it allows some tools to work automatically. port (TCP/UDP) to be listening on both WAN and LAN interfaces. See also:
