...
Non-Kubernetes based service meshes can be installed through infrastructure as code (IaC) products such as [Terraform](/terraform), CloudFormation, ARM Templates, Puppet, Chef, etc.
...
You can use Consul with virtual machines (VMs), containers, or with container orchestration platforms, such as [Nomad](/nomad) and Kubernetes.
