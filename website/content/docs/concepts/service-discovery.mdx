...
You can use Consul with virtual machines (VMs), containers, serverless technologies, or with container orchestration platforms, such as [Nomad](/nomad) and Kubernetes. Consul is platform agnostic which makes it a great fit for all environments, including legacy platforms.
