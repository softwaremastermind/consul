...
- Script + Interval - These checks depend on invoking an external application
  ...
  - [`enable_script_checks`](/consul/docs/agent/config/cli-flags#_enable_script_checks): enable
...
- `Docker + Interval` - These checks depend on invoking an external application which
  ...
  must be configured with [`enable_script_checks`](/consul/docs/agent/config/cli-flags#_enable_script_checks)
...
Review the [service health checks tutorial](/consul/tutorials/developer-discovery/service-registration-health-checks)
to get a more complete example on how to leverage health check capabilities in Consul.
...
Larger outputs are truncated.
The agent must be configured with [`enable_script_checks`](/consul/docs/agent/config/cli-flags#_enable_script_checks)
set to `true` in order to enable Docker health checks.
