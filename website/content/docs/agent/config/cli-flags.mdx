...
- `-retry-join` ((#\_retry_join)) - Address of another agent to join upon starting up. Joining is
  ...
  If your network contains network segments, refer to the [network segements documentation](/consul/docs/enterprise/network-segments/create-network-segment) for additional information.
...
- `-disable-host-node-id` ((#\_disable_host_node_id)) - Setting this to
  ...
  is shared with HashiCorp's [Nomad](/nomad), so if you opt-in
